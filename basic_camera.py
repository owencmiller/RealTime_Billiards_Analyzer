...
def table_detect(image):
    ...
    # image = cv2.reduce(image, 0, cv2.REDUCE_SUM, dtype=cv2.CV_32S)
...
while(True):
    ...
    width = int(circle_frame.shape[1] * scale_percent / 100)
    height = int(circle_frame.shape[0] * scale_percent / 100)
    dim = (width, height)
    resized = cv2.resize(circle_frame, dim, interpolation = cv2.INTER_AREA)
